 ...
 */
const waitUploadDone = identifier =>
  new Promise(resolve => {
    let listener = msg => {
...
const ERROR_IMAGES = {
  gif: {
    cdn_url:
      'https://mmbiz.qlogo.cn/mmbiz_png/wic3OZ3sEjfwAGmvH7C0ROMb9aAfjvickkJI3TurmjVUd20tGB8fd1kgddYI45OkvcrZlvnu4vAjkS0ibSiafHco5g/0?wx_fmt=png',
    cdn_id: 515897144,
    is_placeholder: true
  },
  svg: {
    cdn_url:
      'https://mmbiz.qlogo.cn/mmbiz_png/wic3OZ3sEjfwAGmvH7C0ROMb9aAfjvickkfIVWBgj34x3hIV71YeDr9S8qCHPZquiaIm5db4jcI4s379QcSyCfAsA/0?wx_fmt=png',
    cdn_id: 515897145,
    is_placeholder: true
  },
  webp: {
    cdn_url:
      'https://mmbiz.qlogo.cn/mmbiz_png/wic3OZ3sEjfyrMXEzibBfqZsMhpl1aoibcK5SJ6Aib3exfj1v0UgNrXUvpibU0dwyESN3uHda5PGRwIRnxL8tA8FqSQ/0?wx_fmt=png',
    cdn_id: 515897167,
    is_placeholder: true
  },
  sizeError: {
    cdn_url:
      'https://mmbiz.qlogo.cn/mmbiz_png/wic3OZ3sEjfyrMXEzibBfqZsMhpl1aoibcKj4Cvx37S18WjHrvS8TP4eybIhkdr07B3jzsQbeWUia9hIulnUeNDXSQ/0?wx_fmt=png',
    cdn_id: 515897166,
    is_placeholder: true
  },
  typeError: {
    cdn_url:
      'https://mmbiz.qlogo.cn/mmbiz_png/wic3OZ3sEjfyrMXEzibBfqZsMhpl1aoibcKE20R9gsYOuiaz5AZ1ezUbT7iaIuliawQowL0Dibj3ElvJr70K6Q1ChyicdA/0?wx_fmt=png',
    cdn_id: 515897169,
    is_placeholder: true
  }
}
...
if (themeDOM) {
  let themes = 'xonokai|default|funky|okaidia|solarizedlight|tomorrow|twilight'.split(
    '|'
  )
...
document.addEventListener('paste', e => {
  ...
  Promise.all(promises).then(() => {
    ...
    */
    getAll('img[class*="hash_"', divDOM).forEach(img => {
    ...
    // inject
    const svg = store.get('___startSvg')
    sendMsg(WX_EDITOR_PATTERN, {
      type: 'inject',
      data: svg + divDOM.innerHTML,
...
;(async function init() {
  ...
  const startSvgContent = await fetch('/images/start.svg').then(r => r.text())
  store.set('___startSvg', startSvgContent)
